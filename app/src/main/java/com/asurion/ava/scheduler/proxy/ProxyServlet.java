...
/**
 * ProxyServlet acts as a proxy to access JMX information provided by JDMK HTML adapter which exposes
 * JMX in HTML over HTTP. All "/jmx/*" requests are proxy to "http://localhost:8989". 
 * 
 * @author Scott.Cheng
 * 
 * The proxy servlet is based on:
...
@WebServlet(
        urlPatterns = "/jmx/*",
                initParams =
            {
                @WebInitParam(name = "targetUri", value = "http://localhost:8989"),
...
public class ProxyServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest servletRequest, HttpServletResponse servletResponse)
    ...
    logger.debug("proxyRequestUri ====> "+ proxyRequestUri);
  ...
  protected void copyResponseEntity(HttpResponse proxyResponse, HttpServletResponse servletResponse,
    ...
    */
      ...
      if (entity != null) {
        ...
        String proxyResponseContent = IOUtils.toString(entity.getContent());
        
        String hRefRegex = "HREF=\"/";
        String hRefRep = contextPath!=null? "HREF=\""+ contextPath + "/jmx/": "HREF=\"" + "jmx/";
        proxyResponseContent = proxyResponseContent.replaceAll(hRefRegex, hRefRep);
        
        String actionRegex="ACTION=/";
        String actionRep = contextPath!=null? "ACTION="+ contextPath + "/jmx/": "ACTION="+ "jmx/";
        proxyResponseContent = proxyResponseContent.replaceAll(actionRegex, actionRep);
        
         actionRegex="ACTION=\"/";
         actionRep = contextPath!=null?"ACTION=\""+ contextPath +"/jmx/": "ACTION=\"" +"jmx/";
        proxyResponseContent = proxyResponseContent.replaceAll(actionRegex, actionRep);
        
        //servletOutputStream.write(proxyResponseContent.getBytes(Charset.forName("UTF-8")));
        //servletOutputStream.flush();
        
        //IOUtils.write(proxyResponseContent, servletOutputStream);
        servletResponse.setContentType("text/html");
        servletResponse.setContentLength(proxyResponseContent.length());
        PrintWriter out = servletResponse.getWriter();
        out.write(proxyResponseContent);
        out.close();
  ...
  protected String rewriteUrlFromRequest(HttpServletRequest servletRequest) {
    ...
    pathInfo = pathInfo.replaceFirst(".*jmx", "");
    ...
    if (pathInfo != null) {//ex: /my/path.html
      // getPathInfo() returns decoded string, so we need encodeUriQuery to encode "%" characters
        ...
        /*
        try {
            uri.append(URLEncoder.encode(pathInfo, "UTF-8"));
